...
RUN python3 -m pip install python-multipart passlib[bcrypt] python-jose[cryptography] coloredlogs
...
RUN \
  cd /root/Code/slides/ && \
  cp pvt/conf/conf.json.docker.a vars/conf/conf.json && \
