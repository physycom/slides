...
def data_analysis(df, output):
  ...
  for locName, dfg in df.groupby(['LOC']):
    ...
    curve, = axs.plot(ts, df.SMOOTH, '-o', label=f'{location_map[locName]}', markersize=4)
    curves.append(curve)
    ...
    figg, axxs = plt.subplots(nrows=2, ncols=1, figsize=(16, 10), sharex=True)

    ax = axxs[0]
    ax.plot(tsd, df_daily.SMOOTH, label= 'Data', markersize=4)
    ax.plot(np.unique(tsd), np.poly1d(np.polyfit(tsd, df_daily.SMOOTH, 1))(np.unique(tsd)), c = 'r', label = 'Trend')#), label = fr'Slope = {slope:.3f} $\pm$ {std_err:.3f} rad')
    ...
    ax = axxs[1]
    width = 18*3600
    ax.bar(tsd, df_daily.INCREMENT, width = width, label='test')
    ...
    ptitle = f'Trend in "{location_map[locName]}" @ {freq}'
    figg.suptitle(ptitle, y=0.98)
    trend = f'{output}/trend'
    if not os.path.exists(trend): os.mkdir(trend)
    figg.savefig(f'{trend}/trend_{locName}_{freq}.png')
    plt.close()
    ...
    for week_num, df_week in df_time.groupby(['WEEK_NUMBER']):
      if week_num == args.weekNumber:
        ...
        for data, df_data in df_week.groupby([df_week.index.date]):
          ...
          if wday == args.weekDay:
            ...
            if delta_time >= 17999:
              ...
              if freq == '300s':
                ax.set_ylim(30, 160)  # Grouped by 5min
              elif freq == '900s':
                ax.set_ylim(100, 350)  # Grouped by 15min
            ...
            if delta_time >= 17999:
              ...
              if freq == '300s':
                ax.set_ylim(-40, 140)  # Grouped by 5min
              elif freq == '900s':
                ax.set_ylim(-20, 60)  # Grouped by 15min
          ...
          if delta_time >= 17999:
            curve, = axs.plot(ts, df_data.COUNTER, '-o', label=f'{wday}, {df_data.index.date[0]}', markersize=4)
            axs.set_ylim(0, 1550) 
          else:  
            curve, = axs.plot(ts, df_data.SMOOTH, '-o', label=f'{wday}, {df_data.index.date[0]}', markersize=4)
            if freq == '300s':
              axs.set_ylim(0, 170)  # Grouped by 5min
            elif freq == '900s':
              axs.set_ylim(60, 350)  # Grouped by 15min
          ...
          axs.set_xticks(ts_ticks)
          axs.set_xticklabels(ts_lbl, rotation=45, ha='right')
          axs.set_ylabel(f'Number of People [sampled {freq}]')
        ...
        if args.show:
          ...
          time_gap = f'{all_week_day}/{alfa_time}_{omega_time}'
          if not os.path.exists(time_gap): os.mkdir(time_gap)
          plt.savefig(f'{time_gap}/week_num_{week_num}_{locName}_{freq}.png')
