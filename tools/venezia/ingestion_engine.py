...
class ingestion:
  ...
  def __init__(self, configfile):
    ...
    try:
      ...
      query = f"SELECT CAM_NAME, BARRIER_NAME, DIRECTION, UID FROM {db_name}.BarriersMeta"
      ...
      query = f"SELECT CAM_NAME, UID FROM {db_name}.CameraMeta"
      df = pd.read_sql(query, con=db)
      #print(df)
      self.cam_proxy = {}
  ...
  def push_db(self, config, tag='db', cdata=None, bdata=None):
    ...
    self.logger.info(f'connected to {tag}')

    if cdata:
      try:
        query = f"""INSERT IGNORE INTO {config['database']}.CameraCnt (TIMESTAMP, DATETIME, CAM_UID, MEAN, MAX, MIN) VALUES (%s, %s, %s, %s, %s, %s)"""
    ...
    if bdata:
      try:
        query = f"""INSERT IGNORE INTO {config['database']}.BarriersCnt (TIMESTAMP, DATETIME, BARRIER_UID, COUNTER) VALUES (%s, %s, %s, %s)"""
