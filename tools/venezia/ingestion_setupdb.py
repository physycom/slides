...
if __name__ == '__main__':
  ...
  parser.add_argument('-db', '--dbtag', help='dbtag in config json', required=True)
  ...
  query = f"""
  CREATE TABLE IF NOT EXISTS {db_name}.BarriersMeta (
  ...
  query = f"""
  CREATE TABLE IF NOT EXISTS {db_name}.CameraMeta (
    `UID` INT NOT NULL PRIMARY KEY UNIQUE KEY COMMENT 'Unique id',
    `CAM_NAME` VARCHAR(20) NOT NULL COMMENT 'Camera Name',
  ...
  query = f"""INSERT IGNORE INTO {db_name}.BarriersMeta (UID, CAM_NAME, BARRIER_NAME, DIRECTION, TAIL_LAT, TAIL_LON, TAIL_PX, TAIL_PY, FRONT_LAT, FRONT_LON, FRONT_PX, FRONT_PY) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"""
  cursor.executemany(query, bardata)
  query = f"""INSERT IGNORE INTO {db_name}.CameraMeta (UID, CAM_NAME, LAT, LON) VALUES (%s, %s, %s, %s)"""
  cursor.executemany(query, camdata)
  db.commit()

  query = f"""
  CREATE TABLE IF NOT EXISTS {db_name}.BarriersCnt (
  ...
  query = f"""
  CREATE TABLE IF NOT EXISTS {db_name}.CameraCnt (
    `ID` INT NOT NULL AUTO_INCREMENT PRIMARY KEY COMMENT 'Record internal id',
    `TIMESTAMP` INT UNSIGNED NOT NULL COMMENT 'Record unix timestamp',
