...
if __name__ == '__main__':
  ...
  base_save = os.path.join(os.environ['WORKSPACE'], 'slides', 'work_lavoro', 'dubrovnik', 'data_analysis')
  ...
  try:
   if args.db == 'mysql':
      ...
      for locName, dfg in df.groupby(['LOC']):
        ...
        trend = os.path.join(os.environ['WORKSPACE'], 'slides', 'work_lavoro', 'dubrovnik', 'data_analysis', 'trend')
        ...
        for wday, dfw in df.groupby(['DAY']):
          ...
          s_date = pd.to_datetime(str(dfw.last('7D').index.date[0]) + ' ' + '00:00:00')
          e_date = pd.to_datetime(str(dfw.last('7D').index.date[0]) + ' ' + '23:59:59')
          ...
          comparison = os.path.join(os.environ['WORKSPACE'], 'slides', 'work_lavoro', 'dubrovnik', 'data_analysis', 'comparison')
